...
CREATE INDEX IF NOT EXISTS idx_clothing_season ON clothing(season);
...
CREATE INDEX IF NOT EXISTS idx_clothing_image_clothing ON clothing_image(clothing_id);

DO '
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints
        WHERE constraint_name = ''fk_clothing_main_image''
    ) THEN
...
-- 衣物 <-> 标签（M:N）
CREATE TABLE IF NOT EXISTS clothing_tag (
    clothing_id  BIGINT NOT NULL REFERENCES clothing(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_outfit_favorite ON outfit(is_favorite);
...
CREATE INDEX IF NOT EXISTS idx_calendar_entry_date ON calendar_entry(entry_date);
...
CREATE INDEX IF NOT EXISTS idx_wear_log_clothing ON wear_log(clothing_id);
CREATE INDEX IF NOT EXISTS idx_wear_log_worn_at ON wear_log(worn_at);
CREATE INDEX IF NOT EXISTS idx_wear_log_calendar ON wear_log(calendar_entry_id);
