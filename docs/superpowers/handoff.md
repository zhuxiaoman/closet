...
**下一步：T8**（Category 实体 + Mapper + Service + Controller，TDD）
...
2. Category 是最简的 CRUD（无父子、无用户），按 TDD 顺序写测试 + 实现
3. 完成后用 `mvn -o test` 跑测试验证
4. commit

**T8 提示**：
- 用 `mvn` 不是 `./mvnw`
- 实体类放 `backend/src/main/java/com/closet/entity/Category.java`
